...
clean <- read.xlsx("data/OTM_website-data.xlsx", detectDates = TRUE)
...
# Get a two-year comparison set
startdate <- as.Date("2021-01-01")
enddate <- as.Date("2022-12-31")
compare <- clean %>% filter(authored >= startdate & authored <= enddate)

# Narrow on our desired date range - 2022
...
creative_europe_projects <- read.xlsx("data/CREATIVE-EUROPE_project-funding_2022-12-30.xlsx",
                                      detectDates = TRUE)
