...
df <- clean %>% select(destination_country, id) %>% separate_rows(destination_country, sep=", ")
df$destination_country <- trimws(df$destination_country)
df <- df %>% group_by(destination_country) %>% count()

test <- clean %>% select(online_destination, id)
test <- test %>% group_by(online_destination) %>% count()
